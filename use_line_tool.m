...
set1_back_right_line_y = set1_back_right_line_y + size_of_images(1);
%% Plot base image
figure
% subplot(2,3,1)
% subplot(1,2,1)
imshow(set1_back_left_image)
hold on;
plot(set1_epipolar_x, set1_epipolar_y, '.', 'MarkerSize', 10);
...
hold on;
plot(set1_front_left_line(:,1), set1_front_left_line(:,2))

subplot(2,3,4)
imshow(set1_left_image)
hold on;
plot(set1_left_line(:,1), set1_left_line(:,2))

subplot(2,3,5)
imshow(set1_right_image)
hold on;
plot(set1_right_line(:,1), set1_right_line(:,2))